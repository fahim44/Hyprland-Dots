...
class PrayTimes:
    ...
    def __init__(self, method="Karachi"):
        ...
        # initialize settings
        self.calcMethod = method if method in self.methods else "Karachi"
...
def get_location():
    ...
    loc = data["loc"].split(",")
    tzs = data["timezone"]
    timezone = calculate_offset(tzs)
    return float(loc[0]), float(loc[1]), timezone


def calculate_offset(tzs):
    tz = pytz.timezone(tzs)
    nw = datetime.now()
    offsetSeconds = tz.utcoffset(nw).total_seconds()
...
if __name__ == "__main__":
    latitude, longitude, timezone = get_location()
    times = prayTimes.getTimes(date.today(), (latitude, longitude), timezone)
    data = {}
    data["text"] = "🕋"
    output = ""
    for name, value in times.items():
        output = output + "<b>" + name + "</b> : " + value + "\n"
    data["tooltip"] = output
